...
class SalaScreen extends ConsumerWidget {
  ...
  Future<void> getSala() async {
    String url = '${dotenv.env['HOST_REST']}get_asignarHSala.php';
    try {
  ...
  Future<bool> eliminarAsignarHS(String foliop, String idSala) async {
    var url =
        '${dotenv.env['HOST_REST']}delete_asignarHSala.php?Id_sala=$idSala&Folio=$foliop'; // Reemplaza con la URL del archivo PHP en tu servidor
    var response = await http.post(Uri.parse(url));
    if (response.statusCode == 200) {
